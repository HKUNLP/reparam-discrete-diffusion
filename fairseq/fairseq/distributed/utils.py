...
def _infer_torch_distributed_launch_init(cfg: DistributedTrainingConfig):
    ...
    # see https://github.com/pytorch/fairseq/issues/3318
    # device_id are manually set so that fairseq could 
    # init GPUs correctly under torchrun launching.
    if os.environ.get("CUDA_VISIBLE_DEVICES", None):
        cfg.device_id = int(os.environ["CUDA_VISIBLE_DEVICES"].split(',')[int(os.environ["LOCAL_RANK"])])
    else:
