...
def average_checkpoints(inputs):
    ...
    ema_params_dict = collections.OrderedDict()
    params_keys = None
    ema_params_keys = None
    new_state = None
    ...
    for fpath in inputs:
        ...
        model_ema_params = None
        if "extra_state" in state and "ema" in state["extra_state"]:
            model_ema_params = state["extra_state"]["ema"]
            model_ema_params_keys = list(model_ema_params.keys())
            if ema_params_keys is None:
                ema_params_keys = model_ema_params_keys
            elif ema_params_keys != model_ema_params_keys:
                raise KeyError(
                    "For checkpoint {}, expected list of params: {}, "
                    "but found: {}".format(f, ema_params_keys, model_ema_params_keys)
                )

            for k in ema_params_keys:
                p = model_ema_params[k]
                if isinstance(p, torch.HalfTensor):
                    p = p.float()
                if k not in ema_params_dict:
                    ema_params_dict[k] = p.clone()
    ...
    if ema_params_keys is not None:
        averaged_ema_params = collections.OrderedDict()
        for k, v in ema_params_dict.items():
            averaged_ema_params[k] = v
            if averaged_ema_params[k].is_floating_point():
                averaged_ema_params[k].div_(num_models)
            else:
                averaged_ema_params[k] //= num_models
        new_state["extra_state"]["ema"] = averaged_ema_params
